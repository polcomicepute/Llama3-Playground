...
for obj_name, coords in data.items():
    if 'None' in coords.values() or '' in coords.values() or -1000 in coords.values():
        continue
    else:
        # 모든 객체에 대해 바운딩 박스를 그리기 
        x_min, y_min, x_max, y_max = map(float, [coords['x_min'], coords['y_min'], coords['x_max'], coords['y_max']])
